...
We already named the app to be `redwood-tutorial-postgresql`, so the app we are build will have the correct name. To switch the database we need to edit one line in the file [schema.prisma](https://github.com/redwoodjs/redwood-tutorial/blob/main/api/db/schema.prisma) to be come:
